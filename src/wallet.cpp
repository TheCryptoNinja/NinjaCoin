...
// NINJA: optional setting to unlock wallet for block minting only;
...
// NINJA: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // NINJA: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // NINJA: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // NINJA: not avatar mode
                    {
...
// NINJA: create coin stake transaction
...
// NINJA: check 'spent' consistency between wallet and txindex
// NINJA: fix wallet spent state according to txindex
...
// NINJA: disable transaction (only for coinstake)
