...
public:
    ...
    {
        // NINJA: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    int64 GetMinFee(unsigned int nBlockSize=1, bool fAllowFree=false, enum GetMinFee_mode mode=GMF_BLOCK) const
    {
        ...
        if (fAllowFree)
        {
            if (nBlockSize == 1)
            {
                // Transactions under NINJA are free
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // NINJA: get transaction coin age
...
public:
    ...
    // NINJA: block signature - signed by coin base txout[0]'s owner
    ...
    // NINJA: two types of block: proof-of-work or proof-of-stake
    ...
    // NINJA: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // NINJA: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // NINJA: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // NINJA: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // NINJA: block index flags
    enum  
    {
